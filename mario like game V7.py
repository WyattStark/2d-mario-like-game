...
def game(level):
    ...
    if level==5:
        boxes = [pygame.Rect(300+i*100, GROUND_HEIGHT -100, BOX_SIZE, BOX_SIZE) for i in range(9)]
        enemy_speed=13
    if level==4:
    ...
    while True:
        ...
        if not boxes:
            ...
            max_unlocked_level=max(max_unlocked_level, level+1)
            if level==5: victory_screen(score); return
            else: level_complete(score, level+1); return
        ...
        screen.blit(font.render(f"FPS: {int(clock.get_fps())}",True,WHITE), (SCREEN_WIDTH -80,10))
    ...
    while True:
        ...
        for event in pygame.event.get():
            if event.type==pygame.QUIT: pygame.quit(); exit()
                ...
                if btn.collidepoint(event.pos): game(next_level); return
        clock.tick(60)
...
def victory_screen(score):
    ...
    while True:
        screen.fill((0,150,0))
        screen.blit(font.render("🎉 YOU WIN! 🎉",True,WHITE),(SCREEN_WIDTH//2 -100,100))
        screen.blit(font.render(f"Score: {score}",True,WHITE),(SCREEN_WIDTH//2 -50,160))
        pygame.draw.rect(screen, GREEN, btn)
        screen.blit(font.render("Menu",True,BLACK),(btn.x+btn.width//2 -30, btn.y+btn.height//2 -15))
        pygame.display.flip()
